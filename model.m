...
EquationAppliedOnData = 'EquationAppliedOnData.csv';
ReadEquation = csvread(EquationAppliedOnData);
emotion = importdata('emotion.csv');
ReadEmotion = grp2idx(emotion);
...
X_train = X(rand_num(1:32),:);
y_train = y(rand_num(1:32),:);

X_test = X(rand_num(32:end),:);
y_test = y(rand_num(32:end),:);
...
uistack(h1,'bottom');
uistack(h2,'bottom');
legend([hgscatter;h_sv],{'Angry','Happy','support vectors'})